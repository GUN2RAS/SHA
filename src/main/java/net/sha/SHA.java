...
public class SHA implements ClientModInitializer {
    ...
    public static java.util.function.LongConsumer compileTimerCallback = null;

    @Override
    public void onInitializeClient() {
        if (!net.fabricmc.loader.api.FabricLoader.getInstance().isModLoaded("sodium")) {
            throw new RuntimeException("Mod 'Sodium Hologram API' (sha) requires 'sodium' on the client!");
        }
        net.sha.api.SHAHologramManager.registerClientDelegate(new net.sha.client.SHAHologramManagerClient());
        // бля ну тут долго ебались, инициализация физики
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            if (client.level != null && !client.isPaused() && CLIENT_WORLD_ID != -1) {
                try {
                    // костыль ебаный, 3 степа нужно иначе дергается как сука
                    net.sha.api.physics.RapierEngine.step(CLIENT_WORLD_ID);
                    net.sha.api.physics.RapierEngine.step(CLIENT_WORLD_ID);
                    net.sha.api.physics.RapierEngine.step(CLIENT_WORLD_ID);
                } catch (Exception e) {
                    // похуй, глотаем эксепшены, сил уже нет с этим разбираться
                }
            }
        });
        ...
        ClientPlayNetworking.registerGlobalReceiver(VirtualStructureSyncPayload.ID, (payload, context) -> {
            context.client().execute(() -> {
                ...
                if (entity instanceof VirtualStructureEntity vse) {
                    ...
                    // костыль для синков, энтити еще не заспавнился на клиенте
    ...
    // кэш через ThreadLocal потому что содиум параллельно билдит чанки и без этого оно тихо портит данные без единой ошибки
    // три дня искал баг который оказался race condition в этом месте. три. дня. гемини предлагал synchronized блок, ебанутый
