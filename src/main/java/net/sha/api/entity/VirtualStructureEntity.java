...
public class VirtualStructureEntity extends Entity implements net.sha.api.HologramProvider {
    ...
    public volatile boolean cacheMirageActive = false;
    public volatile int cacheBlockX = 0;
    public volatile int cacheBlockY = 0;
    public volatile int cacheBlockZ = 0;
    ...
    @Override
    public void tick() {
        this.cacheMirageActive = this.getAuthorityOwner() != null && this.getAuthorityOwner().startsWith("portal_mirage");
        net.minecraft.core.BlockPos bp = this.blockPosition();
        this.cacheBlockX = bp.getX();
        this.cacheBlockY = bp.getY();
        this.cacheBlockZ = bp.getZ();
    ...
    public void setAuthorityOwner(String uuid) {
        ...
        this.cacheMirageActive = uuid.startsWith("portal_mirage");
    }
    ...
    public BlockState getSpoofedBlock(int x, int y, int z) {
        ...
        if (this.cacheMirageActive) return null;
        
        BlockPos local = new BlockPos(x - this.cacheBlockX, y - this.cacheBlockY, z - this.cacheBlockZ);
        return this.blockMatrix.get(local);
    }

    @Override
    public boolean isActive() {
        return this.cacheMirageActive && !this.isRemoved();
    }
